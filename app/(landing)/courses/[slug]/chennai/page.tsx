...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  return {
    title: `${baseTitle} in Chennai | Digital Ghuru`,
    description: `Join the best ${baseTitle} in Chennai at Digital Ghuru. Live practical training at our Anna Nagar & Korattur campuses with 100% placement support.`,
  };
}

export default async function ChennaiCoursePage({
  params,
}: {
  params: { slug: string };
}) {
  ...
  // Inject "in Chennai" for SEO and local context
  finalCourse = {
    ...finalCourse,
    title: `${finalCourse.title} in Chennai`,
    subtitle: finalCourse.subtitle ? `${finalCourse.subtitle} at our Anna Nagar & Korattur Campuses` : "Practical Training in Chennai",
  };
