...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-[#fafafa] selection:bg-brand-blue/20 font-sans overflow-hidden">
      {/* Admin Sidebar */}
      <aside className="w-64 bg-[#006FFF] shrink-0 hidden md:flex flex-col">
        <div className="p-6 border-b border-white/20 shrink-0">
          <Link href="/admin/dashboard" className="block bg-white rounded-xl p-3 hover:scale-[1.02] transition-transform shadow-md">
            <Image 
        ...
        <div data-lenis-prevent="true" className="flex-1 overflow-y-auto p-6 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            <div className="flex items-center gap-4 mb-6 pb-6 border-b border-white/20">
              <div className="h-12 w-12 rounded-full bg-white/20 flex items-center justify-center text-white font-display font-bold text-xl shadow-sm">
                {(session.user.name || "A").charAt(0).toUpperCase()}
              </div>
              <div>
                <p className="font-heading font-semibold text-white line-clamp-1">{session.user.name}</p>
                <p className="font-heading text-xs font-semibold text-white/80 uppercase tracking-[0.15em]">Admin</p>
              </div>
            </div>
